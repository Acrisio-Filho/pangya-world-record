...
- `community`: inteiro do voto da comunidade — `0` não votado, `1` aprovado (volta p/ `pending` p/ final do admin), `-1` rejeitado (continua `approved` do admin, fora do index/best). O admin pode forçar `1` via edição. Só com `1` o record entra no index e conta p/ `is_best`.
