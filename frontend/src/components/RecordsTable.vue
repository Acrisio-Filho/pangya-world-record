...
const columnCount = computed(() => 3 + (props.showUser ? 1 : 0) + (props.showStatus ? 1 : 0) + (slots.actions ? 1 : 0));
...
<template>
  <div v-if="!records.length" class="card py-10 text-center text-sm text-slate-500">{{ emptyText }}</div>
        ...
        <tbody>
          ...
          <tr class="border-b border-slate-800/60 hover:bg-slate-900/40">
            <td class="px-3 py-2.5 font-semibold text-slate-100 whitespace-nowrap">
              {{ r.score }}
              <span v-if="r.is_best === 'TRUE'" class="badge-best ml-1">★ BEST</span>
            </td>
          ...
          <tr v-if="isExpanded(r.id)" :id="`record-details-${r.id}`" class="record-details-row border-b border-slate-800/60 last:border-0">
            <td :colspan="columnCount" class="px-3 py-3">
              <dl class="record-details-grid">
                <div><dt>Pang</dt><dd>{{ r.pang ?? "—" }}</dd></div>
                <div><dt>Método</dt><dd>{{ mlabel(r.method) }}</dd></div>
                <div><dt>Vento</dt><dd>{{ wlabel(r.wind) }}</dd></div>
                ...
                <div class="record-details-wide"><dt>Provas</dt><dd><ProofLinks :video="r.video_url" :screenshot="r.screenshot_url" :by="r.nickname || r.user_id" /></dd></div>
              </dl>
            </td>
          </tr>
          </template>
      ...
      <div v-for="r in records" :key="r.id" class="card">
        <div class="flex items-start justify-between gap-2">
          <div class="text-lg font-bold text-slate-100">
            {{ r.score }}
            <span v-if="r.is_best === 'TRUE'" class="badge-best ml-1 align-middle">★ BEST</span>
          </div>
        ...
        <div class="mt-3 flex items-center justify-between gap-2">
          <ProofLinks :video="r.video_url" :screenshot="r.screenshot_url" :by="r.nickname || r.user_id" />
