...
async function search(page = 1) {
  ...
  tb.innerHTML = rows.map(r => `<tr><td>${esc(r.score)}${r.is_best === "TRUE" ? `<span class="best">BEST</span>` : ""}</td><td>${esc(r.pang ?? "")}</td><td>${esc(mlabel(r.method))}</td><td>${esc(wlabel(r.wind))}</td><td><a href="profile.html?id=${esc(r.user_id)}">${esc(r.nickname || r.user_id)}</a></td><td>${esc(cname(r.course_id))}</td><td>${esc(bname(r.powerband_id))}</td><td>${esc(r.power_value ?? "")}</td><td>${esc(fmtDate(r.submitted_at))}</td><td>${r.video_url ? `<a href="${esc(safeUrl(r.video_url))}" target="_blank" rel="noopener" data-by="${esc(r.nickname || r.user_id)}">vídeo</a>` : ""} ${r.screenshot_url ? `<a href="${esc(safeUrl(r.screenshot_url))}" target="_blank" rel="noopener" data-by="${esc(r.nickname || r.user_id)}">print</a>` : ""}</td></tr>`).join("") || `<tr><td colspan="10">Nenhum record.</td></tr>`;
