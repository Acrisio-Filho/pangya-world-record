...
1. `Users` → `id | nickname | email | pass_hash | role | status | bio | youtube_url | points | created_at | google_sub`
...
2. Primeiro admin: cadastre-se pelo site (`register.html`) e depois edite sua linha na aba `Users`: `role=admin`, `status=active`.
3. Extensões > Apps Script > apague tudo > cole o `Code.gs` desta pasta.
...
6. Em `frontend/js/config.js`: `URL_API` = URL `/exec`, `ORIGEM_TOKEN` e `GOOGLE_CLIENT_ID` = os mesmos do passo 4.
